...
from HeroMusic.core.userbot import assistants
from HeroMusic.misc import SUDOERS, pymongodb
from HeroMusic.plugins import ALL_MODULES
from HeroMusic.utils.database import (get_global_tops,
                                       get_particulars, get_queries,
                                       get_served_chats,
                                       get_served_users, get_sudoers,
                                       get_top_chats, get_topp_users)
from HeroMusic.utils.decorators.language import language, languageCB
from HeroMusic.utils.inline.stats import (back_stats_buttons,
                                           back_stats_markup,
                                           get_stats_markup,
...
async def overall_stats(client, CallbackQuery, _):
    if CallbackQuery.from_user.id not in SUDOERS:
        return await CallbackQuery.answer(
            "ᴏɴʟʏ ғᴏʀ sᴜᴅᴏ ᴜsᴇʀs ғᴜᴄᴋ ᴏғғ", show_alert=True
        )
    callback_data = CallbackQuery.data.strip()
    what = callback_data.split(None, 1)[1]
    if what != "s":
